...
class RateLimitError(RetryError):
    ...
    def __init__(self, message: str, retry_after: float = None, last_error: Exception = None, attempt: int = 0):
...
class FreeUsageLimitError(RetryError):
    ...
    def __init__(self, message: str = "Free usage exceeded, add credits", last_error: Exception = None, attempt: int = 0):
        super().__init__(message, last_error, attempt)
...
class RetryConfig:
    ...
    def __init__(
        ...
        non_retryable_errors: tuple = (ContextOverflowError, FreeUsageLimitError),
    ):
...
def calculate_retry_delay(
    ...
    if response_headers:
        ...
        if retry_after:
            ...
            try:
                http_date_delay = (time.mktime(time.strptime(retry_after, "%a, %d %b %Y %H:%M:%S GMT")) - time.time()) * 1000
...
def is_retryable_error(error: Exception) -> Optional[str]:
    """Check if an error is retryable and return reason string."""
